...
df <- scf_stack %>%
      ...
      select(year, hh_id, imp_id, agecl, edcl, race,
  ...
  for(i in 1:2){
    ...
    if(quantile_prob != 0){
      to_plot <- df %>%
        ...
        group_by(year, race, group_var) %>%
        summarise(
          percentile = wtd.quantile(var_for_qtile, weights = wgt, probs=quantile_prob)
        ) %>%
        ungroup() %>%
        gather(-year, -race, -group_var, key=key, value=value) %>%
        arrange(year, race, group_var) %>%
        mutate(wealth_gap = case_when(
          lag(race) != race ~ value/lag(value, 1),
          TRUE ~ NA
        )) %>%
    ...
    file_path <- paste0(out_path, "/", var, "_", quantile_prob_string, "_wb_gap_by_year_", end_filename, ".jpeg")
