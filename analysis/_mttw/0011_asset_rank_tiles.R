...
folder_name <- "_mttw/0011_asset_rank_tiles"
...
create_tiles <- function(file_in, color_list, file_out, size_small, size_large){
  
  rets <- read_excel(paste0(importdir, "/", folder_name, "/", file_in)) %>%
    filter(year>= 2007) %>%
    arrange(year, ret)
  
  n_assets <- length(unique(rets$asset_tw))
  ...
  year_tiles <- data.frame(year = c(seq(first_year, last_year), "年化")) %>%
  ...
  for(i in 1:(n_assets+1)){
    ...
    plot <- ggplot(to_plot, aes(x=year, y=rank, fill=asset_tw)) +
      geom_tile() +
      geom_text(data=to_plot, aes(x=year, y=rank), 
                label = to_plot$text, 
                size = ifelse(is.na(to_plot$asset_tw), size_large, size_small)) +
...
create_tiles("jpm_asset_returns_2004_2018.xlsx", full_colors_10, "assets", 1.6, 2.5)
# ############################  End  ################################## #