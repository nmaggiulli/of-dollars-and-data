...
avc <- readRDS(paste0(localdir, "37-avocado-prices-1966-2016.Rds")) %>%
          mutate(ret_avc = ret) %>%
          select(year, ret_avc)
wht <- readRDS(paste0(localdir, "37-wheat-prices-1966-2016.Rds")) %>%
  mutate(ret_wht = ret) %>%
  select(year, ret_wht)

# Create a weight for avc and wht
w_avc <- 0.5
w_wht <- 1 - w_avc

merged_avc_wht <- left_join(avc, wht) %>%
                    mutate(ret_at = w_avc * ret_avc + w_wht * ret_wht)

first_year <- min(merged_avc_wht$year)
...
merged_avc_wht <- merged_avc_wht %>%
                      left_join(cpi, by = c("year" = "year")) %>%
                      mutate(ret_at = ret_at - rate_cpi,
                             year= as.Date(paste0("1/1/", year) ,"%m/%d/%Y")) %>%
                      select(year, ret_at)
...
# Define the min and max years
min_year <- min(year(full_bv_returns$year))
max_year <- max(year(full_bv_returns$year))


full_bv_returns <- left_join(full_bv_returns, merged_avc_wht)
...
all_at        <- data.frame(matrix(nrow = 1, ncol = 0))
...
all_at        <- find_ret_sd_sharpe(all_at)
...
plot <- ggplot(eff, aes(x = sd, y = exp_return)) + geom_point(alpha = .1, color = ealdark) +
  ...
  # Add all AT
  geom_point(data = all_at, aes(x = sd, y = exp_return), color = "darkgreen", size = 2) +
  geom_text_repel(data = all_at, label = "Avocado Toast Only", family = "my_font", size = 3, nudge_x = -0.015, max.iter = 5000) +
...
file_path = paste0(exportdir, "37-at-index/bv-efficient-frontier.jpeg")
...
for (n in portfolio_list){
  ...
  if (n == "optimal" ){
    ...
    portfolio_name <- "Avocado Toast Only"
  }
...
# Set the file_path based on the function input 
file_path = paste0(exportdir, "37-at-index/bv-drawdowns-less-risky.jpeg")

top_title <- "Maximum Portfolio Losses\n Across All Simulations"

# Limit to the less risky portfolios first
dd_plot <- dplyr::filter(dd_stack, portfolio != "S&P 500 Only" & portfolio != "Avocado Toast Only")
...
# Set the file_path based on the function input 
file_path = paste0(exportdir, "37-at-index/bv-drawdowns-more-risky.jpeg")

top_title <- "Avocado Toast is Very Risky by Itself"

# Limit to the less risky portfolios first
dd_plot_pre <- dplyr::filter(dd_stack, portfolio != "Equal Weighted" & portfolio != "50-50 Stock/Bond")

dd_plot <- dplyr::filter(dd_plot_pre, value > -4 * 10^6)
...
write.csv(results_df, paste0(exportdir, "37-at-index/portfolio-stats.csv"))
