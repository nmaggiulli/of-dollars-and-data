...
folder_name <- "0061_sp500_returns_pe"
...
for (i in 1:nrow(sp500_ret_pe)){
  for (x in returns_to_calc){
    if (i + (x * 12) > nrow(sp500_ret_pe)){
      ...
      sp500_ret_pe[i, name] <- (sp500_ret_pe[(i + (x * 12)), "price_plus_div"]/sp500_ret_pe[(i), "price_plus_div"])
    }
  }
}
...
sp500_ret_pe_long <- sp500_ret_pe %>%
                      ...
                      filter(!is.na(value)) %>%
                      mutate(below_zero = ifelse(value < 1, 1, 0))
...
plot_ret_pe <- function(var){
  ...
  file_path = paste0(exportdir, "0061_sp500_returns_pe/dollar-returns-", var,"-year.jpeg")
  ...
  title <- paste0("Over 30 Years, Lower Starting Valuations\nImply Higher Future Growth")
    ...
    # Set the file_path for the next output
    file_path = paste0(exportdir, "0061_sp500_returns_pe/fit-returns-", var,"-year.jpeg")
...
#
# convert -delay 25 loop -0 dollar-*.jpeg all_plots.gif
