...
file_path <- paste0(out_path, "/ret_sp500_medallion.jpeg")

text_labels <- filter(to_plot, year == max(to_plot$year))
...
plot <- ggplot(data = to_plot, aes(x=year, y = value, fill = key, col = key)) +
  ...
  geom_text_repel(data = text_labels,
                  ...
                  segment.colour = "transparent",
                  max.iter  = 1000,
                  nudge_y = ifelse(text_labels$key == "Medallion Fund", 0.2, -0.1)
  ) +
  scale_color_manual(values = c("black", "blue"), guide = FALSE) +
