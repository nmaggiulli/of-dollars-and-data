...
df <- scf_stack %>%
    ...
    select(year, hh_id, imp_id, 
             agecl, edcl, married, businessowner,
             networth,
             wgt)
...
formulas <- list(
  ...
  Model_3_Partner = "ihs(networth) ~ agecl + edcl + married",
  Model_4_EdMarried  = "ihs(networth) ~ agecl + edcl*married",
  Model_5_Business  = "ihs(networth) ~ agecl + edcl*married + businessowner",
  Model_6_MarriedBusiness = "ihs(networth) ~ agecl + edcl*married + married*businessowner"
)
...
for (name in names(formulas)){
  ...
  model_table[, 1:3] <- round(model_table[, 1:3], 2)
