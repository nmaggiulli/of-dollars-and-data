...
build_and_chart_timing_model <- function(dd_pct){
  ...
  plot <- ggplot(filter(to_plot, key == "index_sp500_tr"), aes(x=date, y=value, col= key)) +
    geom_line(alpha = 0.8) +
    ...
    ggtitle(paste0("S&P 500 Index with Tops and Bottoms\nWhen a ", dd_pct_string, "% Drawdown Occurs")) +
  ...
  # Reset file path
  file_path <- paste0(out_path, "/optimal_timing_dd_pct_", dd_pct_string, ".jpeg")
  
  # Set different note
  note_string   <- str_wrap(paste0("Note:  Assumes no taxes or transaction costs.  When not invested, the perfect timing strategy is in cash."), 
                            width = 85)
  ...
  plot <- ggplot(to_plot, aes(x=date, y=value, col= key)) +
    ...
    of_dollars_and_data_theme +
    ggtitle("Buy and Hold vs. Perfect Timing Strategy\nWith 250 Session (1 Year) Lag") +
    labs(x="Date", y="Portfolio Value",
         caption = paste0("\n", source_string, "\n", note_string))
  ...
  assign(paste0("final_df_", dd_pct_string),final_df, envir = .GlobalEnv)
}

build_and_chart_timing_model(-0.2)
build_and_chart_timing_model(-0.1)
