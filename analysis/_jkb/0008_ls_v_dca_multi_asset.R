...
plot_ls_v_dca <- function(asset, f_out, in_df, var, var_note, invest_dca_cash){
  ...
  to_plot <- in_df %>%
    rename_(.dots = setNames(paste0("ls_", var, "_", n_month_dca, "m"), "Buy Now")) %>%
    rename_(.dots = setNames(paste0("dca_", var, "_", n_month_dca, "m"), "Average-In")) %>%
    ...
    note_string <- str_wrap(paste0("On average, Average-In (over ", n_month_dca, " months) ", perf_string, " Buy Now by ", round(100*avg_performance, 1), "% and ",
                                   "underperforms Buy Now in ", round(100*pct_underperformance, 1), "% of all months shown.  ",
                                   additional_note), 
                            width = 80)
  }
...
run_asset <- function(a, invest_dca_cash){
  ...
  for(i in 1:(length(raw_matrix[,1]) - n_month_dca)){
    ...
    # Calculate the monthly Buy Now returns
    ...
    # Get final Average-In sum
    end_dca <- sum(asset_growth * tbill_growth * (1/n_month_dca))
    
    # Calculate the monthly Average-In returns
  ...
  plot_ls_v_dca(a, out_folder, asset_result, "outperformance", "Average-In outperformance", invest_dca_cash)
}
