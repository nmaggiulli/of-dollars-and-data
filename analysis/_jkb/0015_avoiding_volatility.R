...
folder_name <- "_jkb/0015_avoiding_volatility"
...
spx <- read.csv(paste0(importdir, "/_jkb/0015_avoiding_volatility/SPX_data.csv"),
...
for(i in 1:length(all_years)){
  ...
  if(yr == 2008){
    assign("dd_2008", dd_yr, envir = .GlobalEnv)
  }
...
df <- read.csv(paste0(importdir, "/_jkb/0015_avoiding_volatility/sp500_5yr_treasuries.csv"), skip = 7,
               col.names =  c("date", "ret_sp500", "ret_5yr", "drop")) %>%
  filter(!is.na(ret_sp500)) %>%
  mutate(date = as.Date(date, format = "%m/%d/%Y"),
         year = year(date)) %>%
  filter(year(date) < 2021, year(date) >= min(spx$year)) %>%
  select(-drop) %>%
  left_join(dd_all)
...
for(j in seq(-0.40, -0.05, 0.05)){
  ...
  text_labels[1, "key"] <- "Avoid Drawdowns"
  text_labels[2, "date"] <- as.Date("2003-01-01")
  text_labels[2, "key"] <- "Buy & Hold"
  
  if(j < -0.1){
    text_labels[1, "value"] <- 10000
    text_labels[2, "value"] <- 100
  } else if (j < -0.05){
    text_labels[1, "value"] <- 3000
    text_labels[2, "value"] <- 100
  } else {
    text_labels[1, "value"] <- 12
    text_labels[2, "value"] <- 2000
  }
  
  if(j < -0.14 & j > -0.16){
    assign("dd_15_value", to_plot, envir = .GlobalEnv)
    assign("dts_15", dates, envir = .GlobalEnv)
  }
  ...
  plot <- ggplot(to_plot, aes(x=date, y=value, col = key)) +
    geom_rect(data=dates, aes(xmin = date, ymin = 0, 
                              xmax = end_date, ymax = ymax), fill = bw_colors[1], alpha = 0.2) +
    geom_line() +
    scale_y_continuous(label = dollar, limits = c(0.8, ymax), breaks = c(1, 10, 100, 1000, 10000),  trans = log10_trans()) +
    scale_color_manual(values = c(bw_colors, "transparent"), guide = FALSE) +
