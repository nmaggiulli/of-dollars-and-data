...
plot_percentiles <- function(grepl_string, pct_file_string, percentile_text, var_name, var_slug){
  
  tmp <- cohort_26_41 %>%
    group_by(year) %>%
    rename_(.dots = setNames(var_name, "var_name")) %>%
    summarise(`90th Percentile` = wtd.quantile(var_name, probs = 0.9, weights = wgt),
              `75th Percentile` = wtd.quantile(var_name, probs = 0.75, weights = wgt),
              `50th Percentile` = wtd.quantile(var_name, probs = 0.5, weights = wgt),
              `25th Percentile` = wtd.quantile(var_name, probs = 0.25, weights = wgt),
              `15th Percentile` = wtd.quantile(var_name, probs = 0.15, weights = wgt)) %>%
    ungroup() %>%
  ...
  file_path <- paste0(out_path, "/millennial_cohort_", var_slug, "_by_year_", pct_file_string, "_", max_year, ".jpeg")
  ...
  plot <- ggplot(to_plot, aes(x=year, y = value, col = key)) +
    ...
    ggtitle(paste0("Inflation-Adjusted ", title, " By Year\nHouseholds Age 26-41\n", percentile_text)) +
    labs(x="Year", y=paste0("Inflation-Adjusted ", title),
         caption = paste0(source_string))
  
  if(var_name == "nw" | var_name == "nhd"){
    plot <- plot +
