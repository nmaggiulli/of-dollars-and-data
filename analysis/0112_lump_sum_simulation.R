...
to_plot <- data.frame(month = rep(seq(1,7), 3),
                      ...
                      regime = c(rep("Falling", 7),
                                 rep("Rising", 7),
                                 rep("Flat", 7)),
                      purchase = c(1, rep(0, 6),
                                   1, rep(0, 6),
                                   1, rep(0, 6))
)
...
plot <- ggplot(to_plot, aes(x=month, y=price)) +
  ...
  ggtitle(paste0("DCA vs. Lump Sum Performance\nDuring Different Market Regimes")) +
