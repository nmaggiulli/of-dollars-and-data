...
for (i in loop_list){
  if (i == "data.0.CurrentU90-94"){
    ue_stack <- readRDS(paste0(importdir, "0012_bls_unemployment/bls_unemployment_", i, ".Rds"))
  } else{
    temp <- readRDS(paste0(importdir, "0012_bls_unemployment/bls_unemployment_", i, ".Rds"))
    ue_stack <- rbind(ue_stack, temp)
  }
}

# Load in the area and area_type
area <- readRDS(paste0(importdir, "0012_bls_unemployment/bls_unemployment_area.Rds"))
...
area_type <- readRDS(paste0(importdir, "0012_bls_unemployment/bls_unemployment_area_type.Rds"))
